...
### Aug 23, 2026 — Continuation batch (nomenclature, attendance icon, service CSV key, WhatsApp sender, customer profile)
- **Item 5 — Service category nomenclature (DONE):** standardized 2-level model system-wide. L1 `category` ∈ {Services, Packages}; L2 `sub_category` = real category (Haircut, Hair Spa, Facial…). Fixed Service page (`ServicesModule.js`) to group by L2 + source the CATEGORY dropdown from the classification master (`salon_classification`). Reports "By category" + appointment cards now use L2. Ran idempotent `normalize_service_taxonomy.py` (fixes mislabeled services + populates classification master).
- **Item 2 — Attendance icon in right ribbon (DONE):** new "Mark Attendance" ribbon button (`HomeV2Shell.js`) opens the existing staff quick-attendance drawer (`SalonStaffV3.js`) via event + sessionStorage handoff.
- **Item 4 — Services CSV download/upload with unique key (DONE):** new `GET /salons/{id}/services/export.csv` (first col `service_key` = UUID). `upload-csv` upserts by `service_key` (blank = create + auto-key; existing = update in place). UploadDrawer got a working file upload + "Download existing services" button. Tested round-trip.
- **Item 6 — Default WhatsApp sender + block incoming (DONE):** inbound webhook drops replies for salons without their own active sender (`_salon_has_own_sender`). Default platform sender (8560934455) still used for outbound. MessagesDrawer shows a persistent banner prompting salons to configure their own number.
- **Item 1 — Customer profile drawer (CORE DONE):** `GuestProfileDrawer` (CustomersV2.js) now has Family tab (CRUD, standard relations + Custom), Wallet & Membership tab (balance/history/active membership), online-booking block toggle (Overview), and an Edit button opening the shared CustomerDrawer with ALL fields editable (backend PUT extended: email/anniversary/notes/photo/tags/socials/preferred staff).
- **REMAINING:** Item 1a extras (membership single/family scope + auto-apply membership-discount field beside tip on appointment/invoice); Item 3 (Guests default view setting list/cards/calendar, Modify→edit existing booking via appointment chip, concise columns w/ service-date first).

### Aug 23, 2026 — Batch 2 (membership scope + tip discount, Guests/bookings list)
- **Item 1a — Membership scope + tip discount (DONE):** membership plan editor (MarketingV2.js) now has a Single/Family choice (+ family size); persists via backend `is_family`/`family_size` (verified). Appointment chip (AppointmentDrawer.js) has an editable "Membership discount %" field placed on the same line just before Tip; auto-fills from the customer's active discount membership and folds into the total + order summary.
- **Item 3a/d — Guest (bookings) views (DONE):** QueueTabV2 view switcher already offers List/Cards/Calendar; default changed to **List** (compact one-line view).
- **Item 3c — Concise booking list (DONE):** compact list now has **service Date as the first column**, a 2-line service summary with "+N more", a **Skip** action, **View invoice** for completed bookings, tapping the **customer name opens the customer profile** (reused GuestProfileModal), and tapping the row opens the full booking-detail drawer (services, per-service staff, payment, discounts).
- **Item 3b — Modify booking (PARTIAL):** all Modify buttons now open the appointment chip **pre-filled with the customer** via `salon:open-new-appointment {edit}`; `editToken` prop is plumbed to AppointmentDrawer. REMAINING: saving in-place onto the SAME booking (needs a booking-update backend endpoint + AppointmentDrawer edit-mode); currently Save still creates a new booking.

### Aug 23, 2026 — Item 3b In-place booking edit (DONE)
- New `PUT /salons/{salon_id}/salon-booking/{token_id}` updates the SAME token (services, per-service barber attribution, products w/ inventory delta, total, date/shift/payment) — preserves id/token_number/status/created_at. Verified: edit 1→2 services recomputes total, no duplicate token created.
- AppointmentDrawer now accepts `editToken`, prefills the chip from the booking (services, per-service barber/discount/tier, products, schedule, customer, payment) and Saves via PUT when editing.
- "Modify" everywhere (queue List/Cards + booking detail) opens the prefilled chip; the old add-services modify path is retired from the queue (prop removed).
